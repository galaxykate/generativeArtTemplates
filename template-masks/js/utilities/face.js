...
class Face {
	...
	setToBlend(face0, face1, pct) {
		// console.log(`blend ${face0.idNumber} and ${face1.idNumber}, ${pct}`)

		for (var i = 0; i < this.points.length; i++) {
			this.points[i].setToLerp(face0.points[i], face1.points[i], pct)
		}


		for (var j = 0; j < this.hands.length; j++) {
			let hand = this.hands[j]
			let h0 = face0.hands[j]
			let h1 = face1.hands[j]

			for (var i = 0; i < hand.points.length; i++) {
				hand.points[i].setToLerp(h0.points[i], h1.points[i], pct)
			}
		}
	...
	setToHandsFree(facePoints, handPoints0, handPoints1) {
		...
		for (var i = 0; i < facePoints.length; i++) {
			setPoint(this.points[i], facePoints[i])
		}

		if (handPoints0) {
			this.hands[0].visible = true
			for (var i = 0; i < handPoints0.length; i++) {
				setPoint(this.hands[0].points[i], handPoints0[i])
			}
		} else {
		...
		if (handPoints1) {
			this.hands[1].visible = true
			for (var i = 0; i < handPoints1.length; i++) {
				setPoint(this.hands[1].points[i], handPoints1[i])
			}
		} else {
	...
	testDraw(p, hue) {
		p.text("face", 0, 0)
		p.fill(hue, 100, 50)
		this.points.forEach(pt => p.circle(...pt, 3))
		if (this.hands[0].visible) {
			this.hands[0].points.forEach(pt => p.circle(...pt, 3))
		} else {
			p.text("no hand 0", 200, 100 + this.idNumber*20)
		}
		if (this.hands[1].visible) {
			this.hands[1].points.forEach(pt => p.circle(...pt, 3))
		} else {
			p.text("no hand 1", -200, 100 + this.idNumber*20)
	...
	if (handsfree === undefined) {
		...
		handsfree.use('logger', (data) => {
			...
			if (data.facemesh &&  data.facemesh.multiFaceLandmarks &&  data.facemesh.multiFaceLandmarks.length > 0) {
				let faceMesh = data.facemesh.multiFaceLandmarks?data.facemesh.multiFaceLandmarks[0]:undefined
				let hand0 = data.hands.multiHandLandmarks?data.hands.multiHandLandmarks[0]:undefined
				let hand1 = data.hands.multiHandLandmarks?data.hands.multiHandLandmarks[1]:undefined
				
				hfFace.setToHandsFree(faceMesh, hand0, hand1)
			}
		})
